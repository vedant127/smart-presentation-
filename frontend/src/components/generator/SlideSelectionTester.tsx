...
export const SlideSelectionTester = () => {
    ...
    React.useEffect(() => {
        axios.get('http://localhost:5000/api/presentation-types?isActive=true')
            .then(res => {
                ...
                if (fetchedTypes.length > 0) setSelectedTypeId(fetchedTypes[0]._id);
            })
            .catch(console.error);
    }, []);
    ...
    const handleTestSelection = async () => {
        setLoading(true);
        try {
            const response = await axios.post('http://localhost:5000/api/presentations/generate-presentation', {
                city,
                projectType,
                requirements
            });
